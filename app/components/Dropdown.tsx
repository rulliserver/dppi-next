        ...
        <>
            <Transition
                ...
                <div className={`absolute z-50 rounded-md shadow-lg ${alignmentClasses} ${widthClasses}`} onClick={() => setOpen(false)}>
                    <div className={`rounded-md ring-1 ring-red-500 ring-opacity-5 ` + contentClasses}>{children}</div>
                </div>
            </Transition>
        </>
    );
...
const DropdownLink = ({ className = '', children, ...props }: any) => {
    return (
        <Link
            {...props}
            className={
                'block w-full px-4 py-2 dark:bg-gray-950 text-start text-sm leading-5 text-gray-700 hover:bg-gray-400 dark:hover:text-gray-800 focus:outline-none focus:bg-red-100 transition duration-150 ease-in-out' +
                className
            }>
            {children}
        </Link>
