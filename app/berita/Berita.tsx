                        ...
                        {data?.data.map(({ id, title, photo, slug, view, body, tanggal }: any) => {
                            ...
                            return (
                                <div key={id} className='px-4 py-3 bg-gray-100 dark:text-black rounded-md shadow-sm shadow-red-300 md:grid-cols-'>
                                    ...
                                    <a href={`/berita/${slug}`}>
                                        <Image src={`${BaseUrl}/uploads/${photo}`} width={800} height={400} alt='Photo Berita' className='w-full max-h-[700px] align-middle bg-no-repeat object-cover opacity-90' onError={(e: any) => { e.target.onerror = null; e.target.src = "https://placehold.co/600x400/757575/000000?text=Gagal+Memuat+gambar" }} />
