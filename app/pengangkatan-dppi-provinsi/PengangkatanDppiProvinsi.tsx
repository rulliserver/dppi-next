...
export default function PengangkatanDppiProvinsi() {
    ...
    const [filteredProvinsi, setFilteredProvinsi] = useState<any[]>([]);
    const [selectedProvinsi, setSelectedProvinsi] = useState<string>("");
    const [searchProvinsi, setSearchProvinsi] = useState<string>("");
    ...
    // Filter provinsi based on search
    useEffect(() => {
        if (searchProvinsi) {
            const filtered = provinsi.filter(kab =>
                kab.nama_provinsi.toLowerCase().includes(searchProvinsi.toLowerCase())
            );
            setFilteredProvinsi(filtered);
        } else {
            setFilteredProvinsi(provinsi);
        }
    }, [searchProvinsi, provinsi]);
    ...
    const getProvinsi = () => {
        axios
            ...
            .then((response: any) => {
                ...
                setFilteredProvinsi(response.data);
            })
            .catch((error) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const pendaftaranData = {
                id_provinsi: selectedProvinsiId,
                nama_provinsi: selectedProvinsi,
                        ...
                        <div className="mb-4">
                                ...
                                {showDropdown && (
                                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                        {filteredProvinsi.length > 0 ? (
                                            filteredProvinsi.map((kab) => (
                                                <div
                                                    key={kab.id}
                                                    className="px-4 py-2 hover:bg-gray-100 cursor-pointer border-b border-gray-100"
                                                    onClick={() => handleProvinsiSelect(kab)}
                                                >
                                                    {kab.nama_provinsi}
                                                </div>
                                            ))
                            ...
                            {selectedProvinsi && (
                                <div className="mt-2 text-sm text-gray-600">
                                    Terpilih: <span className="font-semibold">{selectedProvinsi}</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="border-b pb-4">
                                    <h4 className="font-medium text-gray-800 mb-2">Provinsi</h4>
                                    <p className="text-gray-600">{selectedProvinsi}</p>
                                </div>
