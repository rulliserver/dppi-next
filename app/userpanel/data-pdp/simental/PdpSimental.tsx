...
function PdpSimental() {
    ...
    const submitUpdateStatus = async (e: FormEvent) => {
        ...
        if (dataStatus.status === 'Simental') {
            confirmationMessage = 'Anda yakin ingin mengubah status menjadi "Simental"?';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    text: dataStatus.status === 'Simental' ? 'Sedang mengirim email...' : 'Sedang memproses...',
    ...
    const downloadAllExcel = async () => {
        try {
            ...
            let response;
            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                // Gunakan endpoint pdp-simental-all yang mengembalikan SEMUA data sekaligus
                response = await axios.get(`${UrlApi}/adminpanel/pdp-simental-all?${params.toString()}`, {
                    withCredentials: true
                });
            } else {
                // Untuk role lain, tetap gunakan endpoint biasa (mungkin perlu dibuat endpoint all juga)
                response = await axios.get(`${UrlApi}/kesbangpol/pdp-simental-all?${params.toString()}`, {
                    withCredentials: true
                });
            }

            // Untuk pdp-simental-all, response.data adalah array langsung, bukan object pagination
            let allData: PdpData[] = [];

            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                // Endpoint pdp-simental-all mengembalikan array langsung
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'PDP SIMENTAL');

            // Generate nama file
            let fileName = 'PDP_Simental_Semua_Data';
            if (selectedProvinsi) {
