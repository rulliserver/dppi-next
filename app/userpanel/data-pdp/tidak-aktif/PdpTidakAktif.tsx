...
function PdpTidakAktif() {
    ...
    const getPdp = async (page: number = 1, query: string = '', provinsiId: number | '' = '', kabupatenId: number | '' = '') => {
        ...
        try {
            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                const response = await axios.get(`${UrlApi}/adminpanel/pdp-tidak-aktif?${params.toString()}`, {
                    withCredentials: true
                });
                setPdp(response.data);
            } else {
                const response = await axios.get(`${UrlApi}/kesbangpol/pdp-tidak-aktif?${params.toString()}`, {
                    withCredentials: true
                });
                setPdp(response.data);
            }
    ...
    const downloadAllExcel = async () => {
        try {
            ...
            let response;
            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                // Gunakan endpoint pdp-tidak-aktif-all yang mengembalikan SEMUA data sekaligus
                response = await axios.get(`${UrlApi}/adminpanel/pdp-tidak-aktif-all?${params.toString()}`, {
                    withCredentials: true
                });
            } else {
                // Untuk role lain, tetap gunakan endpoint biasa (mungkin perlu dibuat endpoint all juga)
                response = await axios.get(`${UrlApi}/kesbangpol/pdp-tidak-aktif-all?${params.toString()}`, {
                    withCredentials: true
                });
            }

            // Untuk pdp-tidak-aktif-all, response.data adalah array langsung, bukan object pagination
            let allData: PdpData[] = [];

            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                // Endpoint pdp-tidak-aktif-all mengembalikan array langsung
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'PDP TIDAK AKTIF');
        ...
        <div className='dark:bg-slate-900 min-h-screen p-4'>
            ...
            <div className='flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6'>
                <div className='flex items-center mb-4 lg:mb-0'>
                    ...
                    <p className='text-accent mt-1 mx-2 font-bold lg:text-2xl dark:text-white'>DATA PDP TIDAK AKTIF</p>
                </div>
