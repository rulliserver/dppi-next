...
function DataPdp() {
    ...
    const getPdp = async (page: number = 1, query: string = '') => {
        ...
        try {
            const response = await axios.get(`${UrlApi}/userpanel/get-pdp?${params.toString()}`, {
    ...
    const submitUpdateStatus = async (e: FormEvent) => {
        ...
        if (dataStatus.status === 'Verified') {
            Swal.fire({
                icon: 'question',
                text: 'Anda yakin ingin mengubah status menjadi "Verified"?',
                showDenyButton: true,
    ...
    const clickModalDelete = (item: PdpData) => {
        setModalDelete(true);
        setDeleteData({ id: item.id, nama_lengkap: item.nama_lengkap });
    };
    ...
    const deleteSubmit = async (e: any) => {
        ...
        try {
            await axios.delete(`${UrlApi}/adminpanel/pdp/${deleteData.id}`, {
                withCredentials: true,
        ...
        <div className='dark:bg-slate-900 min-h-screen p-4'>
            ...
            <div className='flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6'>
                <div className='flex items-center mb-4 lg:mb-0'>
                    ...
                    <p className='text-accent mt-1 mx-2 font-bold lg:text-2xl dark:text-white'>DATA PDP</p>
                </div>
                                        ...
                                        <td className='border text-center'>
                                            {item.photo ? (
                                                <Image
                                                    ...
                                                    width={80}
                                                    height={120}
                                                    className='w-20 h-auto mx-auto p-0'
                                                    loading='eager'
                                                />
                                            ) : (
                                                <img
            ...
            {/* delete Modal */}
            {modalDelete && (
                <div className='justify-center fixed top-0 left-0 right-0 z-50 p-4 overflow-x-hidden mx-auto md:max-w-1/2 overflow-y-auto md:inset-0 h-modal md:h-full'>
                        ...
                        <div className='w-full mx-auto bg-gray-100 border-2 border-red-200 rounded-md shadow-md dark:bg-default'>
                            <div className='flex flex-col px-4 py-2 rounded-t border-b dark:border-gray-600'>
                                <div className='flex font-semibold text-gray-900 dark:text-white'>
                                    ...
                                    <button type='button' onClick={() => setModalDelete(false)} className='text-gray-400 hover:text-gray-900 rounded-lg p-1.5 ml-auto'>
                            ...
                            <div className='mt-4 px-4'>
                                ...
                                <span className='dark:text-white font-semibold'>{deleteData.nama_lengkap}</span>
                            ...
                            <div className='px-4 mb-4'>
                                <form onSubmit={deleteSubmit}>
                                    <div className='flex flex-row justify-between'>
                                        <button type='button' onClick={() => setModalDelete(false)} className='py-1 px-4 mt-2 bg-yellow-500 rounded-md text-dark'>
                                            Batal
                                        </button>
