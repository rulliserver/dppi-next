...
export default function PelaksanaProvinsi() {
    ...
    const [crop, setCrop]: any = useState({ width: 2, height: 3, aspect: 2/3 });
    ...
    const [cropEdit, setCropEdit]: any = useState({ width: 2, height: 3, aspect: 2/3 });
    ...
    const fetchProvinsi = async () => {
        try {
            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                ...
                const res = await axios.get(`${UrlApi}/provinsi/${user?.id_provinsi}`, {
                    withCredentials: true,
                    headers: { Accept: 'application/json' },
                });
                setProvinsi(res.data || []);
    ...
    const openEdit = (row: Pelaksana) => {
        setDataPelaksana({ id: row.id, nama_lengkap: row.nama_lengkap ?? '', jabatan: row.jabatan ?? '', id_pdp: row.id_pdp ?? '', id_provinsi: row.id_provinsi ?? '' });
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        await axios.put(`${UrlApi}/adminpanel/pelaksana-provinsi/${dataPelaksana.id}`, form, {
            withCredentials: true,
            headers: { Accept: 'application/json' },
        });
        Swal.fire({
    ...
    const handleSubmitCreate = async (e: FormEvent) => {
        ...
        await axios.post(`${UrlApi}/adminpanel/pelaksana-provinsi`, form, {
            withCredentials: true,
                        ...
                        <table className='min-w-full text-left text-sm'>
                            <thead>
                                <tr className='text-center'>
                                    ...
                                    <th className='border py-2 px-2'>Provinsi</th>
                            ...
                            <tbody>
                                ...
                                {items.map((row, idx) => (
                                    <tr key={row.id} className='border-b'>
                                        ...
                                        <td className='py-2 px-2 border text-center'>{row.nama_provinsi || '-'}</td>
                    ...
                    <div className='relative bg-gray-200 rounded-lg shadow-lg dark:bg-default'>
                        <div className='flex items-start justify-between p-4 border-b-2 border-white rounded-t dark:border-gray-600'>
                            <div className='flex font-semibold text-gray-900 dark:text-white '>
                                <span className='mr-1 text-accent'>Tambah Pelaksana Provinsi</span>
                            </div>
            ...
            <div id='editModal' className='fixed top-0 left-0 right-0 z-50 hidden p-4 xl:inset-0 backdrop-blur-sm bg-white/50 min-h-svh'>
                <div className='md:absolute left-0 right-0 relative w-full h-full max-w-xl xl:max-h-175 mx-auto xl:top-20 top-4 xl:h-auto overflow-x-hidden overflow-y-auto'>
                    <div className='relative mx-auto bg-gray-200 rounded-lg shadow-lg dark:bg-default'>
                        <div className='flex items-start justify-between p-4 border-b-2 border-white rounded-t dark:border-gray-600'>
                            <div className='flex font-semibold text-gray-900 dark:text-white '>
                                <span className='mr-1 text-accent'>Edit Pelaksana Provinsi</span>
                            </div>
