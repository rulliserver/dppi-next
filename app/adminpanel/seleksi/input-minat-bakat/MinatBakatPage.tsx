...
export default function MinatBakatPage() {
    ...
    // Minat Bakat States
    const [skor, setSkor] = useState<number>(80);
    ...
    const handleCandidateChange = async (id: number | '') => {
        ...
        try {
            const res = await fetch(`${UrlApi}/seleksi/minat-bakat/${id}`, {
                credentials: 'include'
            });
            if (!res.ok) throw new Error('Gagal memuat detail Minat Bakat');
            const data = await res.json();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${UrlApi}/seleksi/minat-bakat`, {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) throw new Error(result.message || 'Gagal menyimpan minat bakat');

            Swal.fire('Berhasil!', 'Penilaian Minat Bakat berhasil disimpan.', 'success');
    ...
    return (
        <div className="max-w-2xl mx-auto mb-20">
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Input Penilaian Minat Bakat (Seleksi)</h1>
                <p className="text-gray-650 dark:text-gray-400 text-sm">Pencatatan skor minat bakat serta rincian kategori bakat Capaska</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-250 dark:border-gray-800 shadow-sm space-y-6">
                ...
                <div className="flex flex-col gap-1 bg-gray-50 dark:bg-gray-950 p-4 rounded border border-gray-100 dark:border-gray-850">
                    <div className="flex justify-between items-center text-xs font-bold">
                        <span className="text-gray-700 dark:text-gray-300">Skor Evaluasi Minat & Bakat (0 - 100)</span>
                        <span className="text-violet-600 dark:text-violet-400 text-sm">{skor}</span>
                    </div>
                    <input
                        type="range"
                        min="0"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-gray-700 dark:text-gray-300">Kategori / Detail Bakat</label>
                        <input
                            type="text"
                            value={kategori}
                            onChange={(e) => setKategori(e.target.value)}
                            placeholder="Contoh: Kesenian Tari, Olahraga Silat, Musik"
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-gray-700 dark:text-gray-300">Status Kelulusan Tahap Ini</label>
                        <select
                            value={status}
                            onChange={(e) => setStatus(e.target.value)}
                            className="p-2.5 border border-gray-300 dark:border-gray-700 rounded dark:bg-gray-800 dark:text-white text-sm focus:ring-2 focus:ring-violet-500 focus:outline-none"
                        >
                            <option value="Rated">Rated (Selesai)</option>
