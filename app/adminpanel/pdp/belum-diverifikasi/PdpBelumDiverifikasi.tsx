...
function PdpBelumDiverifikasi() {
    ...
    const downloadAllExcel = async () => {
        try {
            ...
            let response;
            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                // Gunakan endpoint pdp-belum-diverifikasi-all yang mengembalikan SEMUA data sekaligus
                response = await axios.get(`${UrlApi}/adminpanel/pdp-belum-diverifikasi-all?${params.toString()}`, {
                    withCredentials: true
                });
            } else {
                // Untuk role lain, tetap gunakan endpoint biasa (mungkin perlu dibuat endpoint all juga)
                response = await axios.get(`${UrlApi}/kesbangpol/pdp-belum-diverifikasi-all?${params.toString()}`, {
                    withCredentials: true
                });
            }

            // Untuk pdp-belum-diverifikasi-all, response.data adalah array langsung, bukan object pagination
            let allData: PdpData[] = [];

            if (user?.role === "Administrator" || user?.role === "Superadmin") {
                // Endpoint pdp-belum-diverifikasi-all mengembalikan array langsung
