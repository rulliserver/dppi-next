...
export default function ProfileUser(props: any) {
	...
	const handleSubmitEdit = async (e: FormEvent) => {
		...
		try {
			...
			const response = await fetch(`${UrlApi}/auth/update-profile`, {
				method: 'put',
				credentials: 'include',
				body: formData
			});
			...
			<div className='px-0 mt-6 lg:px-4 '>
				...
				<div className='flex justify-center'>
					<div className='w-48 h-48 rounded-full overflow-clip'>
						{user.photo ? <img src={`${BaseUrl}/${user.photo}`} alt='User photo' className='w-48 h-auto' /> : <img src='/images/logo.png' alt='Logo' className='w-48 h-auto' />}
					</div>
				</div>
				...
				<div className='max-w-xl mx-auto'>
					<div className='relative grid grid-cols-3 p-2 bg-white border-2 rounded-md dark:bg-slate-700 dark:text-white border-orange-500'>
						...
						{data.geotourism_name ? (
							<>
								<p>Nama Geosite</p>
								<p className='col-span-2'>: {data.geotourism_name}</p>
							</>
						) : (
							''
						)}
			...
			{formEdit && (
				<div className='fixed top-0 left-0 right-0 z-40 p-2 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full'>
					<div className='relative w-full h-full max-w-3xl mx-auto top-20 md:h-auto bg-white rounded-lg shadow-lg dark:bg-slate-800'>
						...
						<div className='border-b dark:border-white0 border-1 border-slate-200'></div>
						<form onSubmit={handleSubmitEdit} encType='multipart/form-data'>
							<div className='px-4 py-2'>
								<div className='grid grid-cols-4 mt-1 mr-2'>
									<InputLabel htmlFor='photo' value='Foto Profile' className='flex py-1 text-sm text-dark dark:text-white' />
								...
								<p className='mb-2 text-xs text-red-500'> *) abaikan jika tidak ada perubahan photo</p>
							...
							<div className='mt-2 border-b dark:border-white0 border-1 border-slate-200'></div>
